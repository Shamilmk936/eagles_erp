...
class _EnqyiryListWidgetState extends State<EnqyiryListWidget> {
  ...
  getPendingEnq() async {
    FirebaseFirestore.instance
        .collection('enquiry').where('status',isEqualTo: 0)
        .orderBy('date', descending: true)
        .limit(limit)
        .snapshots()
        .listen((event) {
  ...
  searchEnq(String search) async {
    enqList=[];
    await enqList.clear();
    print(totalEnq.length.toString()+'              Total');
    print('enqList.length.toString()');
    print(enqList.length.toString());
  for(var searchItem in totalEnq){

    if((!enqList.contains(searchItem))&&searchItem['name'].toUpperCase().toString().contains(search.toUpperCase())){
